...
export type CleanTacticalKitPattern = "plain" | "hoops" | "stripes" | "slash" | "chestDash";
...
function drawKitPattern(
  target: Graphics,
  pattern: CleanTacticalKitPattern,
  accentColor: number,
  radius: number,
  ...
  if (pattern === "hoops") {
    ...
    const dashY = -radius * 0.04;
    target
      .moveTo(-radius * 0.42, dashY)
      .lineTo(radius * 0.42, dashY)
      .moveTo(-radius * 0.24, dashY + radius * 0.26)
      .lineTo(radius * 0.24, dashY + radius * 0.26);
  }
...
export function createCleanTacticalPlayerToken({
  ...
  radius: number;
  kitPattern?: CleanTacticalKitPattern;
  ...
  const ringColor = variant === "phosphor"
    ? mixColor(accentColor, 0xffffff, 0.2)
    : mixColor(primaryColor, accentColor, 0.26);
  ...
  const labelFontSize = isNumericLabel
    ? safeLabel.length >= 2 ? safeRadius * 1.04 : safeRadius * 1.18
    : safeRadius * 0.8;
