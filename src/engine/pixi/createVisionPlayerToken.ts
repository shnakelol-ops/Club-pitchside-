...
export function createVisionPlayerToken({
  ...
  shadow
    .ellipse(0.35, radius * 1.04, radius * 0.86, radius * 0.28)
    .fill({ color: 0x020617, alpha: 0.24 })
    .ellipse(0.35, radius * 0.98, radius * 0.6, radius * 0.19)
  ...
  ambientHalo
    ...
    .fill({ color: teamPalette.halo, alpha: 0.08 })
    .circle(0, 0, radius * 1.1)
    .fill({ color: teamPalette.halo, alpha: 0.07 });
  token.addChild(ambientHalo);

  const activeHalo = new Graphics();
  activeHalo
    .circle(0, 0, radius * 1.36)
    .fill({ color: teamPalette.halo, alpha: 0.24 })
    .circle(0, 0, radius * 1.17)
    .fill({ color: mixColor(teamPalette.halo, 0xffffff, 0.15), alpha: 0.16 });
  activeHalo.alpha = 0;
  ...
  notch
    .poly([0, -radius * 1.08, radius * 0.1, -radius * 0.88, -radius * 0.1, -radius * 0.88])
    .fill({ color: teamPalette.notch, alpha: 0.82 });
  ...
  const labelText = new Text({
    ...
    style: {
      fill: 0xf8fbff,
      fontSize: numericLabel && label.length >= 2 ? radius * 0.58 : radius * 0.64,
      fontWeight: "900",
      ...
      stroke: {
        color: 0x020617,
        width: numericLabel && label.length >= 2 ? 0.65 : 0.58,
        join: "round",
      },
    },
  });
  ...
  const controller: VisionPlayerTokenController = {
    applyState: ({
      ...
      ambientHalo.alpha = active ? 1 : 0.88;
      activeHalo.alpha = active ? 0.32 + pulse * 0.16 : 0;
      const activeScale = active ? 1 + pulse * 0.07 : 1;
