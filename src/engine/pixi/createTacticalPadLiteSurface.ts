...
export async function createTacticalPadLiteSurface(
  ...
  let basicRoutePoints: RoutePoint[] = [];
  ...
  function syncWhiteboardTokenInputMode(): void {
    ...
    const canDragPlayers = activeWhiteboardTool === "move" && !isPlaybackInputLocked() && !isBasicRouteDevMode;
    for (const player of players) {
  ...
  function renderBasicRoutePreview(): void {
    ...
    if (basicRoutePoints.length < 2) return;
    const first = mapper.normalizedToWorld(basicRoutePoints[0]!);
    basicRoutePreviewGraphic.moveTo(first.x, first.y);
    for (let index = 1; index < basicRoutePoints.length; index += 1) {
      const point = mapper.normalizedToWorld(basicRoutePoints[index]!);
  ...
  function appendBasicRoutePoint(point: RoutePoint): void {
    const previous = basicRoutePoints[basicRoutePoints.length - 1];
    if (
      previous &&
    ...
    basicRoutePoints = [...basicRoutePoints, point];
  ...
  function startBasicRouteFollowForSelectedPlayer(options?: { force?: boolean }): boolean {
    ...
    const route =
      basicRoutePoints.length >= 2 ? basicRoutePoints.map((point) => ({ ...point })) : buildDevRouteForPlayer(player);
