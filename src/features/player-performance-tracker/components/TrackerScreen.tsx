...
export default function TrackerScreen(props:{players:TrainingPlayer[];logs:TrainingLogEntry[];elapsedSeconds:number;isRunning:boolean;period:TrainingPeriod;activeEventKey:TrainingEventKey|null;onToggleTimer:()=>void;onReset:()=>void;onPeriod:(p:TrainingPeriod)=>void;onSelectEvent:(k:TrainingEventKey)=>void;onTapPlayer:(id:string)=>void;onDelete:(id:string)=>void;onUndo:()=>void;lastDeleted:TrainingLogEntry|null|undefined}){
  ...
  return <div className="ppt-wrap"><section className="ppt-panel ppt-center"><div className="ppt-eyebrow">MY SQUAD</div><div className="ppt-timer">{fmt(props.elapsedSeconds)}</div><div className="ppt-row"><button className={`ppt-control ${props.isRunning?"ppt-reset":"ppt-start"}`} onClick={props.onToggleTimer}>{props.isRunning?"Pause":"Start"}</button><button className="ppt-control ppt-reset" onClick={props.onReset}>Reset</button></div><div className="ppt-periods">{(["PRE","1H","2H","ET"] as TrainingPeriod[]).map((p)=><button key={p} className={`ppt-period ${props.period===p?"active":""}`} onClick={()=>props.onPeriod(p)}>{p}</button>)}</div><div className="ppt-active">Active: {active?active.label:"None"}</div></section><EventGrid activeEventKey={props.activeEventKey} onSelectEvent={props.onSelectEvent} showShots={showShots} onToggleShots={()=>setShowShots((s)=>!s)} /><PlayerPicker players={props.players} ratings={ratings} onTapPlayer={props.onTapPlayer} /><EventLog logs={filtered} filter={filter} onFilter={setFilter} onDelete={props.onDelete} />{props.lastDeleted && <button className="ppt-undo" onClick={props.onUndo}>↩</button>}</div>;
}
