...
export default function EventGrid({ activeEventKey, onSelectEvent, showShots, onToggleShots }: Props) {
  ...
  return <section><div className="ppt-grid4">{topEvents.map((event)=><EBtn key={event.key} event={event} active={activeEventKey===event.key} onClick={()=>onSelectEvent(event.key)} />)}<button type="button" onClick={onToggleShots} className={`ppt-event ppt-blue ${shotActive?"active":""}`}><div className="ppt-event-label small">SHOT</div><div className="ppt-points">{showShots?"▲":"▼"}</div></button></div>{showShots&&<div className="ppt-grid2" style={{marginTop:8}}>{shotEvents.map((event)=><EBtn key={event.key} event={event} active={activeEventKey===event.key} onClick={()=>onSelectEvent(event.key)} />)}</div>}<div className="ppt-grid3" style={{marginTop:8}}>{remaining.map((event)=><EBtn key={event.key} event={event} active={activeEventKey===event.key} onClick={()=>onSelectEvent(event.key)} />)}</div></section>;
}
