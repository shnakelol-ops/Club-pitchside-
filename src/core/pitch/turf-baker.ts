  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      if (cosineStripes) {
        ...
        const harmonic = r.verticalHarmonicRatio * Math.cos(xf * twoPi * b * 2 + 0.31);
...
export function bakeTurfWashTexture(sport: PitchSport): Texture {
...
export function gaelicStripeTilingParams(
